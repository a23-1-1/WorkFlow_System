...
**典型误配**：在 LLM「上下文」里添加了文档提取器的 `text`，但 USER 为空或未写 `【简历原文】` + `{{#文档提取器.text#}}`。

| 检查项 | 异常表现 | 结果 |
|--------|----------|------|
| 上下文 | 已挂 `text` 或 `extracted_text` | 面板橙字：**「要启用上下文功能，请在提示词中引用上下文变量」** |
| USER | 无 `【简历原文】` 或占位符未替换 | Trace 中 USER 几乎为空 → 模型输出全 `null` / `[]` |
| 代码节点 | 收到合法空 JSON | 仍可能 **`valid=true`**，业务上等于提取失败 |

**推荐修复（方案 A）**：

1. **删除** LLM 上下文中的文档提取器变量（上下文 **留空**）。
2. USER **全文** 粘贴 [`prompt-user-template.txt`](prompt-user-template.txt)，确认 `【简历原文】` 下一行为 `{{#文档提取器.text#}}`（或与你画布一致的占位符）。
3. SYSTEM **仅** [`prompt-system.txt`](prompt-system.txt)，不含简历变量。
4. 重跑 Trace：`【简历原文】` 下方必须是真实正文。

**不要** 只在上下文拖入 `text` 而 USER 为空——这与本仓库默认 **方案 A** 不符，也是「文档提取器有字、输出仍全 null」的最高频原因之一。

#### 橙字「要启用上下文功能…」（补充说明）

Dify 在 LLM 节点 **上下文** 区域引用变量但未在 USER/SYSTEM 中引用上下文变量时，会出现橙色提示。

| 做法 | 说明 |
|------|------|
| **推荐：方案 A** | 删除上下文变量；USER 含 `【简历原文】` + `{{#文档提取器.text#}}`；上下文留空 |
| **备选：坚持走上下文** | 必须在 USER 或 SYSTEM 中显式引用上下文变量（如 `{{#context#}}` 或你版本中的等价写法），并保证 Trace 里模型能读到完整简历 |
...
| 上下文已加但未引用 | 只在上下文挂 `text`，USER 无 `【简历原文】` 正文 | 删除上下文变量；USER 粘贴 [`prompt-user-template.txt`](prompt-user-template.txt)；见上文「上下文已加但未引用」 |
| LLM 橙字「要启用上下文功能…」 | 上下文引用了变量但未在 USER/SYSTEM 引用，或与方案 A 混用 | 方案 A：删除上下文变量，正文只走 USER；或必须在 USER/SYSTEM 引用上下文变量 |
...
2. 上传本地虚构/脱敏样例 docx（勿提交仓库；见 [`.gitignore`](../.gitignore)）。
3. 看结束输出是否含姓名、学校、项目等关键词。
...
针对**校招 / 数据开发**类虚构样例（如 [`examples/output-sample.json`](../examples/output-sample.json) 所描述的结构），LLM 应重点扫描以下区块（原文有什么抽什么，不要臆造）：

| 区块 | 常见位置 / 关键词 | 映射字段 |
|------|-------------------|----------|
| 个人信息 | 姓名、手机、邮箱、意向城市 | `basic.*` |
...
- 若原文确有姓名但 JSON 中 `name` 仍为 null → 几乎可断定 LLM **未看到正文**，回到第一节步骤 1–2。
